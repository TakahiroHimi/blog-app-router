    ...
    <div className="space-y-10">
      {posts.map((post) => (
        <article key={`${post.year}-${post.month}-${post.slug}`} className="border-b border-gray-200 pb-8 last:border-0">
          <Link href={`/posts/${post.year}/${post.month}/${post.slug}`} className="group block">
            <h2 className="text-2xl font-semibold mb-2 group-hover:text-blue-600 transition-colors">{post.title}</h2>
          </Link>
