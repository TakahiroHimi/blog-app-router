...
published: true
---
Next.js app routerで、mdxファイルに書いたコンテンツをページとして表示する方法が複数あり、実装しながらあれこれ悩んだので備忘録として記録しておく。  

## 結論

`gray-matter`を利用して、frontmatterはjsのオブジェクトとして、コンテンツの内容は文字列として読み込む。
...
あとはmarkdownでGitHubと同じ記法を使えるようになる`remark-gfm`と、コードブロックにシンタックスハイライトを適用するための`rehype-pretty-code`を利用する。  
...
export default async function PostPage({ params }: { params: PageParams }) {
  ...
  const code = String(
    await compile(content, {
      ...
      rehypePlugins: [[rehypePrettyCode, { theme: 'dark-plus' }]], // VSCodeのデフォルトと同じテーマを設定
    }),
  )
  ...
  return (
    <article >
      <div className="prose"> {/* @tailwindcss/typographyを適用するためのclassNameを設定 */}
        <MDXContent />
      </div>
    </article>
  )
...
```
※別途`tailwind.config.ts`のpluginで`typography`を設定する

## まとめ
descriptionをfrontmatterに毎回ちゃんと書きさえすれば`@next/mdx`で全然良かったんだけど、ちゃんと書く自信が全く無かったので少し複雑になった。  
ただ最終的に割と納得する形に落ち付けられて良かった。  